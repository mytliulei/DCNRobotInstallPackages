如果安装过程中间有任何问题，请及时与liuleic@digitalchina.com联系。


注意：
1. 如有安装过Python2.7版本则建议先卸载后再安装，安装时请注意选择安装路径（不能有中文不能有空格）

2. 安装后请自行将python安装目录加入到环境变量中(或运行env.bat文件,env.bat只适用于win7,xp,2003,其他系统请手工设置）

3. 阅读完本说明后关闭，看界面提示输入y继续安装，输入n退出


本升级包内容包括：

01-python-2.7.7.msi
02-wxPython2.8-win32-unicode-2.8.12.1-py27.exe
03-setuptools-5.4.2.win32-py2.7.exe
04-robotframework-2.8.6.win32.exe
05-robotframework-ride-1.3.win32.exe
06-pywin32-219.win32-py2.7.exe
07-pyreadline-2.0.win32-py2.7.exe
08-WinPcap_4_1_3.exe
09-dnet-1.12.win32-py2.7.exe
10-pcapy-0.10.8.win32-py2.7.exe
11-pcap-1.1.win32-py2.7.exe
12-pyte-0.4.8.tar.gz
13-pycrypto-2.6.win32-py2.7.exe
14-ecdsa-0.11.tar.gz
15-paramiko-1.14.0.tar.gz
16-robotframework-sshlibrary-2.0.2.tar.gz
17-scapy-master.zip
18-pygal-1.7.0.tar.gz